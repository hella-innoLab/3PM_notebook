...
#res=500
rcopts=" -V- -n ${nprocs} -w- -ab 2 -ad 512 -lw 1.0e-6 -ffc " # low to show...
res=250

# generate suns at patch positions
printf "\n generate suns - we assume we have that from the sensor calculation ... \n"
# printf "\n generating suns ... \n"
...
for var in blinds_20deg_BSDF blinds_20deg_geom ; do
  ...
  printf " calculate direct sun matrix facade for system ${var}... \n"
  time vwrays -ff -vf view/view_fish_p01.vf  -x ${res} -y ${res} | \
    rcontrib ${rcopts} `vwrays -vf view/view_fish_p01.vf -x ${res} -y ${res} -d` \
    -o matrices/img_vmx_sun_fac/tutorial_room_${var}_%04d.hdr -e MF:${mf_sun_pos} -f reinhart.cal -b rbin -bn Nrbins -m solar ${octree}
done
